...
def create_base_map(center=SHENZHEN_CENTER, zoom=DEFAULT_ZOOM, title="深圳出租车GPS分析",
                    show_boundary=True, prefer_canvas=False, show_title=True):
    """创建基础地图"""
    m = folium.Map(
        location=center,
        zoom_start=zoom,
...
def _build_corrected_animation_frames(df, timed_pieces):
    """将校正后的分段坐标展开为带时间的动画帧。"""
    if df.empty:
        return []
    ...
    for piece in timed_pieces or []:
        coords = piece.get('coords') or []
        if not coords:
            continue

        piece_start_time = piece.get('start_time')
        piece_end_time = piece.get('end_time')
        if piece_start_time is not None and piece_end_time is not None:
            start_time = pd.to_datetime(piece_start_time)
            end_time = pd.to_datetime(piece_end_time)
        else:
            times = pd.to_datetime(df['time']).tolist()
            start_idx = int(piece.get('start_row_idx', 0))
            end_idx = int(piece.get('end_row_idx', start_idx))
        ...
        if count == 1 or end_time <= start_time:
            time_values = [start_time] * count
        else:
            total_seconds = (end_time - start_time).total_seconds()
            time_values = [
                start_time + timedelta(seconds=total_seconds * i / (count - 1))
        ...
        for idx, coord in enumerate(coords):
            frames.append({
                'lat': float(coord[0]),
                'lon': float(coord[1]),
                'ts': int(time_values[idx].timestamp() * 1000),
                'time': time_values[idx].strftime('%Y-%m-%d %H:%M:%S'),
            })
    ...
    frames.sort(key=lambda item: (item['ts'], item['lat'], item['lon']))
    return frames


def _build_original_animation_frames(df):
    """将原始 GPS 点展开为带时间的动画帧。"""
    if df.empty:
        return []
...
def _build_aligned_corrected_frames(df, timed_pieces):
    """为每个原始时间点生成一个对齐的校正位置，保证左右按同一时间轴播放。"""
    if df.empty:
        return []

    row_coords = [None] * len(df)
    pieces = timed_pieces or []
    ...
    for piece in pieces:
        ...
        if start_idx == end_idx:
            row_coords[start_idx] = coords[-1]
            continue

        span = end_idx - start_idx
        for offset, row_idx in enumerate(range(start_idx, end_idx + 1)):
            if span == 0:
                coord_idx = len(coords) - 1
            else:
                coord_idx = round(offset * (len(coords) - 1) / span)
            row_coords[row_idx] = coords[coord_idx]

    frames = []
    for idx, (_, row) in enumerate(df.iterrows()):
        ts = pd.to_datetime(row['time'])
        coord = row_coords[idx] if row_coords[idx] is not None else [float(row['lati']), float(row['long'])]
        frames.append({
            'lat': float(coord[0]),
            'lon': float(coord[1]),
            'ts': int(ts.timestamp() * 1000),
...
def _inject_corrected_single_map_animation(m, vehicles_payload, initial_time_text, stats_html, cache_html):
    """给单地图注入校正后轨迹动画。"""
    map_name = m.get_name()
    payload_json = json.dumps(vehicles_payload, ensure_ascii=False)
    initial_time_json = json.dumps(initial_time_text, ensure_ascii=False)
    ...
    <style>
      ...
      .corrected-run-time .label {{ font-size: 12px; color: #475569; }}
      .corrected-run-time .value {{ margin-top: 4px; font-size: 18px; font-weight: 700; color: #0f172a; }}
      .corrected-run-panel {{
        position: fixed;
        right: 20px;
        bottom: 30px;
      ...
      @media (max-width: 960px) {{
        .corrected-run-controls {{ left: 12px; top: 70px; right: 12px; flex-wrap: wrap; }}
        .corrected-run-time {{ right: 12px; top: 74px; }}
        .corrected-run-panel {{ right: 12px; bottom: 12px; width: calc(100vw - 24px); }}
      }}
    </style>
    ...
    <div class="corrected-run-time">
      <div class="label">当前时间</div>
      <div id="corrected-live-time" class="value">{initial_time_text}</div>
    </div>
    <div class="corrected-run-panel">
      <div style="font-weight:700; margin-bottom:6px; color:#065f46;">运行说明</div>
      <div style="font-size:12px; color:#475569;">初始显示原始轨迹与校正后路线；点击开始后，静态校正线隐藏，车辆沿校正路线运行。</div>
    ...
    <script>
    (function() {{
      const vehicles = {payload_json};
      const initialTime = {initial_time_json};
      const correctedColor = '{COLOR_CORRECTED}';
      ...
      function waitForMap() {{
        ...
        function centerOnMarkers() {{
          const activeMarkers = vehicles.map(function(vehicle) {{ return vehicle.marker; }}).filter(Boolean);
          ...
          map.panTo([lat / activeMarkers.length, lon / activeMarkers.length], {{ animate: true, duration: 0.35 }});
        }}
        ...
        function tick() {{
          ...
          vehicles.forEach(function(vehicle) {{
            const correctedFrames = vehicle.corrected_frames || [];
            if (vehicle.index + 1 < correctedFrames.length) {{
              vehicle.index += 1;
              const frame = correctedFrames[vehicle.index];
              vehicle.path.push([frame.lat, frame.lon]);
              vehicle.marker.setLatLng([frame.lat, frame.lon]);
              vehicle.pathLine.setLatLngs(vehicle.path);
              moved = true;
            }}

            const currentFrame = correctedFrames[vehicle.index];
            if (currentFrame && currentFrame.ts >= latestTs) {{
              latestTs = currentFrame.ts;
              latestTime = currentFrame.time || latestTime;
            }}
          }});

          if (timeEl) timeEl.textContent = latestTime;
          centerOnMarkers();
          if (moved) setTimeout(tick, playbackDelay);
        }}
        ...
        if (speedButton) {{
          speedButton.addEventListener('click', function() {{
            ...
            if (playbackDelay === 90) {{ playbackDelay = 45; speedButton.textContent = '倍速 4x'; return; }}
            playbackDelay = 180;
            speedButton.textContent = '倍速 1x';
          }});
        }}
        if (startButton) {{
          startButton.addEventListener('click', function() {{
            if (started) return;
            staticCorrectedLayers.forEach(function(layer) {{ map.removeLayer(layer); }});
            started = true;
...
def _inject_corrected_dual_map_animation(dual_map, vehicles_payload, initial_time_text, stats_html, cache_html):
    """给双地图注入静态总览与分屏运行回放。"""
    left_map_name = dual_map.m1.get_name()
    right_map_name = dual_map.m2.get_name()
    payload_json = json.dumps(vehicles_payload, ensure_ascii=False)
    initial_time_json = json.dumps(initial_time_text, ensure_ascii=False)
    ...
    <style>
      ...
      body.run-mode .comparison-title {{ display: block; }}
      .map-side-label {{
        position: fixed;
        top: 72px;
        z-index: 9999;
      ...
      body.run-mode .map-side-label {{ display: block; }}
      .map-side-label.left {{ left: 24px; color: #9a3412; border: 2px solid #f97316; }}
      .map-side-label.right {{ right: 24px; color: #065f46; border: 2px solid #059669; }}
      .map-status-panel {{
        position: fixed;
        right: 20px;
        bottom: 30px;
      ...
      body.run-mode .map-status-panel {{ display: block; }}
      .map-time-panel {{
        position: fixed;
        right: 20px;
        top: 112px;
      ...
      body.run-mode .map-time-panel {{ display: block; }}
      .map-time-panel .label {{ font-size: 12px; color: #475569; }}
      .map-time-panel .value {{ margin-top: 4px; font-size: 18px; font-weight: 700; color: #0f172a; }}
      #static-overview {{
        position: fixed;
        inset: 0;
        z-index: 10000;
        background: linear-gradient(180deg, rgba(248, 250, 252, 0.97), rgba(255, 255, 255, 0.95));
      }}
      #static-overview-map {{
        position: absolute;
        inset: 0;
      ...
      @media (max-width: 960px) {{
        .map-status-panel {{ width: 300px; max-width: calc(100vw - 24px); right: 12px; bottom: 12px; }}
        .map-time-panel {{ right: 12px; top: 112px; }}
        .map-side-label.left {{ left: 12px; }}
        .map-side-label.right {{ right: 12px; top: 112px; }}
        .overview-panel {{ right: 12px; top: 12px; width: calc(100vw - 24px); }}
      }}
    </style>
    <div id="static-overview">
      <div id="static-overview-map"></div>
      <div class="overview-panel">
        <div style="font-size:20px; font-weight:800; color:#0f172a;">路网校正静态总览</div>
        <div style="margin-top:8px; color:#475569; line-height:1.6; font-size:13px;">先查看原始轨迹与校正轨迹的静态对比。点击下方按钮后，再进入左右分屏运行视图。</div>
        <div style="margin-top:8px; color:#475569; line-height:1.6; font-size:13px;">进入运行视图后，左右两边都会按时间推进，地图会跟随车辆自动移动。</div>
        <div style="margin-top:10px; font-size:12px; color:#6b7280;">橙色虚线 = 原始 GPS，绿色实线 = 校正后路线</div>
        <div style="margin-top:12px; color:#374151; font-size:12px;">{stats_html}</div>
        <div style="margin-top:8px; color:#0f6cbd; font-size:12px;">{cache_html}</div>
        <button id="start-split-run" class="overview-button">进入分屏运行</button>
      </div>
    </div>
    <div class="comparison-title">路网校正轨迹对比</div>
    <div class="map-side-label left">左侧：原始轨迹动态回放</div>
    <div class="map-side-label right">右侧：校正后动态回放</div>
    <div class="map-time-panel">
      <div class="label">当前时间</div>
      <div id="corrected-live-time" class="value">{initial_time_text}</div>
    </div>
    <div class="map-status-panel">
      <div style="font-weight:700; margin-bottom:6px; color:#065f46;">路网校正运行视图</div>
      <div style="font-size:12px; color:#475569;">右侧仅显示车辆实时行驶路径，不显示回退线/调试线。</div>
      <div style="margin-top:8px; color:#374151; font-size:12px;">{stats_html}</div>
      <div style="margin-top:8px; color:#0f6cbd; font-size:12px;">{cache_html}</div>
      <div style="margin-top:8px; font-size:11px; color:#6b7280;">左侧橙色虚线 = 原始 GPS，右侧绿色实线 = 已行驶校正路径</div>
    </div>
    """
    dual_map.get_root().html.add_child(folium.Element(overlay_html))
    ...
    (function() {{
      ...
      const initialTime = {initial_time_json};
      const correctedColor = '{COLOR_CORRECTED}';
      const originalColor = '{COLOR_ORIGINAL}';

      function createStaticOverview() {{
        const el = document.getElementById('static-overview-map');
        if (!el) return null;
        const firstPoint = (vehicles[0] && vehicles[0].original[0]) || (vehicles[0] && vehicles[0].corrected[0]) || [22.52847, 114.05454];
        const staticMap = L.map('static-overview-map', {{ zoomControl: true, preferCanvas: true }}).setView(firstPoint, 12);
        ...
        const bounds = [];
        vehicles.forEach(function(vehicle) {{
          const rawPoints = vehicle.original.map(function(p) {{ return [p[0], p[1]]; }});
          const correctedPoints = vehicle.corrected.map(function(p) {{ return [p[0], p[1]]; }});
          if (rawPoints.length > 1) {{
            L.polyline(rawPoints, {{ color: originalColor, weight: 4, opacity: 0.8, dashArray: '8 6' }}).addTo(staticMap);
            rawPoints.forEach(function(p) {{ bounds.push(p); }});
          }}
          if (correctedPoints.length > 1) {{
            L.polyline(correctedPoints, {{ color: correctedColor, weight: 5, opacity: 0.92 }}).addTo(staticMap);
            correctedPoints.forEach(function(p) {{ bounds.push(p); }});
          }}
        }});
        if (bounds.length) staticMap.fitBounds(bounds, {{ padding: [40, 40], maxZoom: 15 }});
        return staticMap;
      }}

      function centerMapOnActiveMarkers(map, markers) {{
        const active = markers.filter(Boolean);
        ...
        map.panTo([lat / active.length, lon / active.length], {{ animate: true, duration: 0.35 }});
      }}

      function waitForMaps() {{
        if (typeof {left_map_name} === 'undefined' || typeof {right_map_name} === 'undefined') {{
          setTimeout(waitForMaps, 100);
          return;
        }}

        const leftMap = {left_map_name};
        const rightMap = {right_map_name};
        const timeEl = document.getElementById('corrected-live-time');

        vehicles.forEach(function(vehicle) {{
          const originalFrames = vehicle.original_frames || [];
          const correctedFrames = vehicle.corrected_frames || [];
          if (!originalFrames.length && !correctedFrames.length) return;

          const leftStart = originalFrames[0] || correctedFrames[0];
          const rightStart = correctedFrames[0] || originalFrames[0];
          vehicle.leftMarker = L.circleMarker([leftStart.lat, leftStart.lon], {{
            radius: 7,
            color: '#ffffff',
            weight: 2,
            fillColor: originalColor,
            fillOpacity: 1
          }}).addTo(leftMap).bindPopup('车辆 ' + vehicle.vehicle_id + ' 原始轨迹');
          vehicle.rightMarker = L.circleMarker([rightStart.lat, rightStart.lon], {{
            radius: 7,
            color: '#ffffff',
            weight: 2,
            fillColor: correctedColor,
            fillOpacity: 1
          }}).addTo(rightMap).bindPopup('车辆 ' + vehicle.vehicle_id + ' 校正轨迹');

          vehicle.leftPath = [[leftStart.lat, leftStart.lon]];
          vehicle.rightPath = [[rightStart.lat, rightStart.lon]];
          vehicle.leftLine = L.polyline(vehicle.leftPath, {{
            color: originalColor,
            weight: 4,
            opacity: 0.9,
            dashArray: '8 6',
          ...
          vehicle.rightLine = L.polyline(vehicle.rightPath, {{
            color: correctedColor,
            weight: 5,
            opacity: 0.92,
            lineCap: 'round',
        ...
        function stepVehicle(vehicle) {{
          ...
          if (vehicle.leftIndex + 1 < originalFrames.length) {{
            vehicle.leftIndex += 1;
            const leftFrame = originalFrames[vehicle.leftIndex];
            vehicle.leftPath.push([leftFrame.lat, leftFrame.lon]);
            vehicle.leftMarker.setLatLng([leftFrame.lat, leftFrame.lon]);
            vehicle.leftLine.setLatLngs(vehicle.leftPath);
            moved = true;
          }}

          if (vehicle.rightIndex + 1 < correctedFrames.length) {{
            vehicle.rightIndex += 1;
            const rightFrame = correctedFrames[vehicle.rightIndex];
            vehicle.rightPath.push([rightFrame.lat, rightFrame.lon]);
            vehicle.rightMarker.setLatLng([rightFrame.lat, rightFrame.lon]);
            vehicle.rightLine.setLatLngs(vehicle.rightPath);
            moved = true;
          }}
        ...
        function tick() {{
          ...
          vehicles.forEach(function(vehicle) {{
            if (stepVehicle(vehicle)) moved = true;
            if (vehicle.leftMarker) leftMarkers.push(vehicle.leftMarker);
            if (vehicle.rightMarker) rightMarkers.push(vehicle.rightMarker);
            const leftFrame = (vehicle.original_frames || [])[vehicle.leftIndex];
            const rightFrame = (vehicle.corrected_frames || [])[vehicle.rightIndex];
            if (leftFrame && leftFrame.ts >= latestTs) {{
              latestTs = leftFrame.ts;
              latestTime = leftFrame.time || latestTime;
            }} else if (rightFrame && rightFrame.ts >= latestTs) {{
              latestTs = rightFrame.ts;
              latestTime = rightFrame.time || latestTime;
            }}
          }});

          if (timeEl) timeEl.textContent = latestTime;
          centerMapOnActiveMarkers(leftMap, leftMarkers);
          centerMapOnActiveMarkers(rightMap, rightMarkers);

          if (moved) setTimeout(tick, 280);
        }}

        const overviewMap = createStaticOverview();
        const startButton = document.getElementById('start-split-run');
        if (startButton) {{
          startButton.addEventListener('click', function() {{
            document.body.classList.add('run-mode');
            const overview = document.getElementById('static-overview');
            const coordPanel = document.getElementById('coord-panel');
    ...
    """
    dual_map.get_root().html.add_child(folium.Element(animation_js))
...
def plot_corrected_trajectory(vehicle_ids, start_time=None, end_time=None,
    ...
    for vehicle_id in vehicle_ids:
        ...
        all_results.append((vehicle_id, result))
    ...
    for vehicle_id, orig in all_original:
        folium.PolyLine(
            orig,
    ...
    for index, (vehicle_id, orig) in enumerate(all_original):
        ...
        debug_segments = all_debug_segments[index][1]
        result = all_results[index][1]
        result_df = result.get('df')
        timed_pieces = result.get('timed_pieces', [])

        if not timed_pieces and corr_segments:
            timed_pieces = [
                {'start_row_idx': 0, 'end_row_idx': max(len(result_df) - 1, 0), 'coords': corr, 'break_before': False}
            ]
        ...
        vehicles_payload.append({
            ...
            'original': orig,
            'corrected': corr,
            'original_frames': _build_original_animation_frames(result_df),
            'corrected_frames': _build_corrected_animation_frames(result_df, timed_pieces),
            'debug_count': len(debug_segments),
        })

    initial_time_text = ''
    for vehicle in vehicles_payload:
        if vehicle['corrected_frames']:
            initial_time_text = vehicle['corrected_frames'][0]['time']
            break
        if vehicle['original_frames']:
            initial_time_text = vehicle['original_frames'][0]['time']
            break

    _inject_corrected_single_map_animation(
        m,
        vehicles_payload,
        initial_time_text or '—',
        stats_html,
        cache_html,
    )
    return m
